...
import {
    EFFICIENTDET_COLORS_ARRAY,
    EFFICIENTDET_LABELS_ARRAY,
} from "../efficientDetLabels";
...
export function createEfficientDetDetector(
  ...
  return (frame: Frame, mirrored: boolean): Detection[] => {
    "worklet";
    ...
    const boxes = new Float32Array(out[0]!); // [1,100,4] -> flat 400
    const classes = new Float32Array(out[1]!); // [1,100]
    const scores = new Float32Array(out[2]!); // [1,100]
    const nDet = new Float32Array(out[3]!)[0] | 0;
