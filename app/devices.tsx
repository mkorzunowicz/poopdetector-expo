...
function Device({ device, onPress, theme }: DeviceProps): React.ReactElement {
  const photoResolutions = useMemo(
  ...
  const maxPhotoRes = useMemo(
    () =>
      photoResolutions.reduce((prev, curr) => {
        if (curr.width * curr.height > prev.width * prev.height) return curr;
        return prev;
      }, photoResolutions[0]),
    [photoResolutions],
  );
  const maxVideoRes = useMemo(
    () =>
      videoResolutions.reduce((prev, curr) => {
        if (curr.width * curr.height > prev.width * prev.height) return curr;
        return prev;
      }, videoResolutions[0]),
    [videoResolutions],
  ...
  const deviceTypes = useMemo(() => {
    const types = device.physicalDevices.map(
      (physicalDevice) => physicalDevice.type,
    );
    return (types.length > 0 ? types : [device.type]).join(" + ");
  }, [device]);
  const maxFps = useMemo(
    () =>
      device.supportedFPSRanges.reduce(
        (currentMax, range) => Math.max(currentMax, range.max),
        0,
      ),
    [device],
  );
      ...
      {maxPhotoRes != null && (
        <View style={styles.horizontal}>
          ...
          <Text style={styles.resolutionText}>
            {maxPhotoRes.width}x{maxPhotoRes.height}
          </Text>
        </View>
      )}
      {maxVideoRes != null && (
        <View style={styles.horizontal}>
          ...
          <Text style={styles.resolutionText}>
            {maxVideoRes.width}x{maxVideoRes.height} @ {Math.round(maxFps)} FPS
          </Text>
        </View>
      )}
